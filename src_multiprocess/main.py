...
def simulate_object_list(Com_obj_list_view:VIEW_t):
    global latposition, longposition

    # Ensure the object list is initialized
    if not hasattr(Com_obj_list_view, "object_list_for_draw") or Com_obj_list_view.object_list_for_draw is None:
        Com_obj_list_view.object_list_for_draw = [None] * 30  # Initialize with 30 empty slots

    # Simulate message counter
    Com_obj_list_view.MsgCntr = random.randint(1, 100)  # Random message counter (1-100)
    
    # Simulate objects in the list
    #for i in range(len(Com_obj_list_view.object_list_for_draw)):
    for i in range(1):
        ...
        Com_obj_list_view.object_list_for_draw[i] = object_list_for_draw_t(
            object_id=i,
...
def communication_process(shm_obj_name, shm_ego_name):
    Com_obj_list_view:VIEW_t
    # Access the shared memory
    shm_obj = SharedMemory(name=shm_obj_name)
    shm_ego = SharedMemory(name=shm_ego_name)
    serialized_obj_data = bytes(shm_obj.buf[:])  # Read serialized data from shared memory
    Com_obj_list_view = pickle.loads(serialized_obj_data)  # Deserialize the data
    serialized_egoM_data = bytes(shm_ego.buf[:])  # Read serialized data from shared memory
    Com_EgoMotion_data = pickle.loads(serialized_egoM_data)  # Deserialize the data

    can_bus_radar, can_bus_car, dbc = init_com()  # Initialize the CAN communication
    if can_bus_radar is None or can_bus_car is None:
        print("Failed to initialize CAN buses. Exiting...")
        return
    ...
    while True:
        ...
        simulate_object_list(Com_obj_list_view)

        # Process the Tx data
        process_tx(can_bus_radar, can_bus_car)

        # Process the Rx data
        process_rx(dbc, can_bus_radar, can_bus_car, Com_obj_list_view, Com_EgoMotion_data)

        # Serialize and update shared memory
        serialized_obj_data = pickle.dumps(Com_obj_list_view)
        shm_obj.buf[:len(serialized_obj_data)] = serialized_obj_data
        
        # Serialize and update shared memory
        serialized_egoM_data = pickle.dumps(Com_EgoMotion_data)
        shm_ego.buf[:len(serialized_egoM_data)] = serialized_egoM_data

def draw_process(shm_obj_name, shm_ego_name):
    Draw_obj_list_view:VIEW_t
    Draw_EgoMotion_data:egomotion_t
    ...
    while running:
        # Access the shared memory
        serialized_obj_data = bytes(shm_obj.buf[:])  # Read serialized data from shared memory
        Draw_obj_list_view = pickle.loads(serialized_obj_data)  # Deserialize the data
        
        serialized_egoM_data = bytes(shm_ego.buf[:])  # Read serialized data from shared memory
        Draw_EgoMotion_data = pickle.loads(serialized_egoM_data)  # Deserialize the data
        ...
        # Draw vehicles
        if Draw_obj_list_view.MsgCntr > 0:
            for obj in Draw_obj_list_view.object_list_for_draw:
                if obj.object_id != INVALID_OBJECT_ID:  # Ensure object_id is valid
                    update_vehicle(screen, obj, vehicle_group)

        # Use the menu state
        if is_rays_enabled[0]:
            draw_rays(screen, ego_vehicle, vehicle_group)

        draw_extraInfo(screen, Draw_EgoMotion_data, vehicle_group, Draw_obj_list_view.ScanID)
...
def main():
    try:
        # Serialize the initial ObjList_VIEW
        initial_obj_list = ObjList_VIEW
        serialized_data = pickle.dumps(initial_obj_list)
        
        # Serialize the initial EgoMotion_data
        initial_ego_motion = EgoMotion_data
        serialized_ego_motion = pickle.dumps(initial_ego_motion)

        # Create shared memory for ObjList_VIEW
        shm_obj = SharedMemory(create=True, size=len(serialized_data))
        shm_obj.buf[:len(serialized_data)] = serialized_data  # Write serialized data to shared memory

        # Create shared memory for EgoMotion_data
        shm_ego = SharedMemory(create=True, size=len(serialized_ego_motion))
        shm_ego.buf[:len(serialized_ego_motion)] = serialized_ego_motion  # Write serialized data to shared memory
        
        # Start the processes
        p_draw = multiprocessing.Process(target=draw_process, args=(shm_obj.name, shm_ego.name))
        p_com = multiprocessing.Process(target=communication_process, args=(shm_obj.name, shm_ego.name))
        p_draw.start()
        p_com.start()
     ...
     #   with ProcessPoolExecutor(max_workers=2) as executor:
            # Submit the communication process to one core
     #       comm_future = executor.submit(communication_process, shm.name)
            # Submit the drawing process to another core
     #       draw_future = executor.submit(draw_process, shm.name)
