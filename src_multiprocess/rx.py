...
def process_rx(dbc, can_bus_radar, can_bus_car, Com_obj_list_view:VIEW_t, Com_EgoMotion_data:egomotion_t):
            ...
            for entry in list_of_Object_attr:
                if(entry.arbitration_id == message_radar.arbitration_id):
                    if e2e.p05.e2e_p05_check(message_radar.data, message_radar.dlc, data_id=entry.e2e_DataId) is True:
                        ...
                        Com_obj_list_view.MsgCntr = decoded_message.get_signal_by_name(entry.MsgCntr)
                        Com_obj_list_view.ScanID = decoded_message.get_signal_by_name(entry.ScanID)
                        ...
                        Com_obj_list_view.object_list_for_draw[index_entry] = object_list_for_draw_t(
                        ...
                        Com_obj_list_view.object_list_for_draw[index_entry + 1] = object_list_for_draw_t(
    ...
    try:
        ...
        if message_car.arbitration_id == VEHICLE_SPEED:
            Com_EgoMotion_data.Speed = dbc.get_message_by_frame_id(message_car.arbitration_id).get_signal_by_name('FLR2RdrVehicleSpeed')
        
        if message_car.arbitration_id == WHEEL_SPEED:
            Com_EgoMotion_data.Left_wheel_speed = dbc.get_message_by_frame_id(message_car.arbitration_id).get_signal_by_name('FLR2RdrLeftWheelSpeed')
            Com_EgoMotion_data.Right_wheel_speed = dbc.get_message_by_frame_id(message_car.arbitration_id).get_signal_by_name('FLR2RdrRightWheelSpeed')
            #Com_EgoMotion_data.YawRate = db.get_message_by_frame_id(message_car.arbitration_id).get_signal_by_name('FLR2RdrYawRate')
            #Com_EgoMotion_data.LatAcc = db.get_message_by_frame_id(message_car.arbitration_id).get_signal_by_name('FLR2RdrLatAcc')
            #Com_EgoMotion_data.LongAcc = db.get_message_by_frame_id(message_car.arbitration_id).get_signal_by_name('FLR2RdrLongAcc')
    ...
    return Com_obj_list_view.ScanID
