...
def _menu_get_font(size):
    if size not in _menu_font_cache:
        _menu_font_cache[size] = menu_font.Font(menu_font.get_default_font(), size)
    return _menu_font_cache[size]
...
is_rays_enabled = [
    True
]  # Use a mutable object to allow modification inside the action function
# Screen mode state
...
def toggle_can_screen_pause():
    ...
    if is_can_screen_paused[0]:
        ...
        paused_messages = (
            can_sniffer.messages.copy() if hasattr(can_sniffer, "messages") else []
        )
    else:
...
def draw_simple_checkbox(
    screen: menu_Surface, x, y, size, is_checked, color, action, label=""
):
    ...
    if is_checked:
        menu_draw.line(
            screen, color, (x + 4, y + size // 2), (x + size // 3, y + size - 4), 3
        )
        menu_draw.line(
            screen, color, (x + size // 3, y + size - 4), (x + size - 4, y + 4), 3
        )
...
def draw_extraInfo(screen: menu_Surface, EgoMotion_data_local, vehicle_group, scanID):
    ...
    text = font.render(
        "Nb of objects: " + str(len(vehicle_group)) + " ScanID: " + str(scanID),
        True,
        white,
    )
    text_rect = text.get_rect()
...
def draw_can_screen_toggle_button(
    screen: menu_Surface, x, y, width, height, color, label="CAN Data"
):
    """Draws an exit button and calls exit_callback when clicked."""
    rect = menu_draw.rect(screen, color, (x, y, width, height), border_radius=6)
...
def draw_swipe_instructions(
    screen: menu_Surface, is_can_screen=False, is_radar_status_screen=False
):
...
def draw_can_data_screen(screen: menu_Surface):
    ...
    if is_can_screen_paused[0]:
        title_text = title_font.render(
            "CAN Message Monitor - PAUSED", True, (255, 255, 0)
        )  # Yellow when paused
    else:
        title_text = title_font.render(
            "CAN Message Monitor - LIVE", True, (0, 255, 0)
        )  # Green when live
    ...
    # Draw separator line
    menu_draw.line(
        screen, white, (20, header_y + 25), (screen.get_width() - 20, header_y + 25), 2
    )
    ...
    messages_to_display = (
        paused_messages if is_can_screen_paused[0] else can_sniffer.messages
    )
    max_messages = min(
        len(messages_to_display), (screen.get_height() - start_y - 100) // line_height
    )
    ...
    if is_can_screen_paused[0]:
        ...
        status_text = (
            f"Live: {len(can_sniffer.messages)} messages - Press SPACE to pause"
        )
...
def draw_radar_status_screen(screen, radar_signal_status, events):
    ...
    if radar_signal_status is None:
        ...
        no_data_info = data_font.render(
            "Waiting for CAN ID 0x45 message...", True, yellow
        )
        ...
        counter_text = small_font.render(
            f"Cnt: {radar_signal_status.counter}", True, white
        )
        ...
        sys_text = small_font.render(
            f"Sys: {'OK' if not radar_signal_status.sys_fail_flag else 'FAIL'}",
            True,
            sys_fail_color,
        )
        ...
        rdr_text = small_font.render(
            f"Rdr: {'ON' if radar_signal_status.rdr_sts else 'OFF'}",
            True,
            rdr_sts_color,
        )
        ...
        trans_text = small_font.render(
            f"TX: {'ON' if radar_signal_status.rdr_trans_act else 'OFF'}",
            True,
            rdr_trans_color,
        )
        ...
        temp_color = (
            green
            if radar_signal_status.internal_temp < 80
            else (yellow if radar_signal_status.internal_temp < 90 else red)
        )
        temp_text = small_font.render(
            f"Temp: {radar_signal_status.internal_temp:.0f}°C", True, temp_color
        )
        ...
        ts_text = small_font.render(
            f"TS: {'OK' if radar_signal_status.timestamp_status else 'BAD'}",
            True,
            ts_color,
        )
        ...
        ub_text = small_font.render(
            f"UB: {'OK' if not radar_signal_status.signal_status_ub else 'ERR'}",
            True,
            signal_ub_color,
        )
        ...
        scan_text = small_font.render(
            f"Scan: {radar_signal_status.scan_id_sts}", True, white
        )
        ...
        cal_status_color = (
            green
            if radar_signal_status.cal_sts == 3
            else (yellow if radar_signal_status.cal_sts == 2 else red)
        )
        cal_names = {0: "None", 1: "InProg", 2: "Part", 3: "Done"}
        cal_text = small_font.render(
            f"Cal: {cal_names.get(radar_signal_status.cal_sts, 'Unk')}",
            True,
            cal_status_color,
        )
        ...
        cal_result_color = (
            green
            if radar_signal_status.cal_rlt_sts == 0
            else (yellow if radar_signal_status.cal_rlt_sts == 1 else red)
        )
        cal_result_names = {0: "OK", 1: "Warn", 2: "Err", 3: "Crit"}
        result_text = small_font.render(
            f"Res: {cal_result_names.get(radar_signal_status.cal_rlt_sts, 'Unk')}",
            True,
            cal_result_color,
        )
        ...
        prog_text = small_font.render(
            f"Prog: {radar_signal_status.cal_prgrss_sts}", True, white
        )
        screen.blit(prog_text, (col2_x, y2))
        y2 += line_height

        wheel_text = small_font.render(
            f"WhlF: {radar_signal_status.whl_comp_fact:.2f}", True, white
        )
        screen.blit(wheel_text, (col2_x, y2))
        y2 += line_height + 3

        # Software versions
        sw_text = small_font.render(
            f"SW: {radar_signal_status.sw_vers_major}.{radar_signal_status.sw_vers_minor}",
            True,
            cyan,
        )
        ...
        if_text = small_font.render(
            f"IF: {radar_signal_status.if_vers_major}.{radar_signal_status.if_vers_minor}",
            True,
            cyan,
        )
        ...
        blockage_color = (
            green
            if radar_signal_status.blockage == 0
            else (yellow if radar_signal_status.blockage <= 2 else red)
        )
        block_names = {0: "None", 1: "Part", 2: "Sig", 3: "Full"}
        block_text = small_font.render(
            f"Blk: {block_names.get(radar_signal_status.blockage, 'Unk')}",
            True,
            blockage_color,
        )
        ...
        interference_color = (
            green
            if radar_signal_status.interference == 0
            else (yellow if radar_signal_status.interference <= 2 else red)
        )
        int_names = {0: "None", 1: "Low", 2: "Med", 3: "Hi"}
        int_text = small_font.render(
            f"Int: {int_names.get(radar_signal_status.interference, 'Unk')}",
            True,
            interference_color,
        )
        ...
        speed_text = small_font.render(
            f"Spd: {radar_signal_status.ego_spd_est:.2f}m/s", True, white
        )
        screen.blit(speed_text, (col3_x, y3))
        y3 += line_height

        yaw_text = small_font.render(
            f"Yaw: {radar_signal_status.ego_yaw_rate_est:.3f}r/s", True, white
        )
        ...
        x_text = small_font.render(
            f"X: {radar_signal_status.x_axis_offs:.2f}", True, white
        )
        screen.blit(x_text, (col3_x, y3))
        y3 += line_height

        y_text = small_font.render(
            f"Y: {radar_signal_status.y_axis_offs:.2f}", True, white
        )
        screen.blit(y_text, (col3_x, y3))
        y3 += line_height

        z_text = small_font.render(
            f"Z: {radar_signal_status.z_axis_offs:.2f}", True, white
        )
        ...
        x_orient_text = small_font.render(
            f"X: {radar_signal_status.x_orient_ang:.0f}", True, white
        )
        screen.blit(x_orient_text, (col3_x, y3))
        y3 += line_height

        y_orient_text = small_font.render(
            f"Y: {radar_signal_status.y_orient_ang:.0f}", True, white
        )
        screen.blit(y_orient_text, (col3_x, y3))
        y3 += line_height

        z_orient_text = small_font.render(
            f"Z: {radar_signal_status.z_orient_ang:.0f}", True, white
        )
        ...
        flt_text = small_font.render(
            f"Flt: {radar_signal_status.flt_reason}", True, flt_color
        )
        screen.blit(flt_text, (col4_x, y4))
        y4 += line_height

        comm_color = green if radar_signal_status.comm_flt_reason == 0 else red
        comm_text = small_font.render(
            f"Comm: {radar_signal_status.comm_flt_reason}", True, comm_color
        )
        screen.blit(comm_text, (col4_x, y4))
        y4 += line_height

        int_sts_text = small_font.render(
            f"IntSts: {radar_signal_status.rdr_int_sts}", True, white
        )
        ...
        azi_text = small_font.render(
            f"Az: {radar_signal_status.azi_ang_cor:.1f}", True, white
        )
        screen.blit(azi_text, (col4_x, y4))
        y4 += line_height

        ele_text = small_font.render(
            f"El: {radar_signal_status.ele_ang_cor:.1f}", True, white
        )
        ...
        timestamp_text = small_font.render(
            f"TS: {radar_signal_status.timestamp}", True, white
        )
