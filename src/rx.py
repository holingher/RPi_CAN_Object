...
DEFAULT_RADAR_MESSAGE = Message(
    arbitration_id=0x000, data=[], dlc=0, is_extended_id=False, is_fd=True
)
...
class CanSnifferData:
    ...
    def add_message(self, arbitration_id: int, data: bytes, timestamp: float = None):
        ...
        timestamp_str = (
            f"{time.time():.3f}" if timestamp is None else f"{timestamp:.3f}"
        )
        data_hex = data.hex().upper() if data else "00"
        message_str = (
            f"ID: 0x{arbitration_id:03X} | Data: {data_hex} | Time: {timestamp_str}"
        )
...
class FlrFlr1canFr96:
    ...
    counter: int = (
        0  # FLR2SignalStatusCounter: 16|8@1+ (1.0,0.0) [0.0|255.0] "Unitless"
    )

    # Temperature and Environment
    internal_temp: float = (
        -40.0
    )  # FLR2SignalStatusInternalTemp: 24|15@1+ (0.01,-40.0) [-40.0|215.0] "degC"
    timestamp: int = (
        0  # FLR2SignalStatusTimeStamp: 56|64@1+ (1.0,0.0) [0.0|0.0] "Milliseconds"
    )

    # Position and Orientation Offsets
    y_axis_offs: float = (
        -25.0
    )  # FLR2SignalStatusYAxisOffs: 120|16@1+ (0.00145,-25.0) [-25.0|25.0] "m"
    z_axis_offs: float = (
        -25.0
    )  # FLR2SignalStatusZAxisOffs: 136|16@1+ (0.00145240648,-25.0) [-25.0|25.0] "m"

    # Speed and Motion Estimation
    ego_spd_est: float = (
        0.0  # FLR2SignalStatusEgoSpdEst: 152|15@1+ (0.00391,0.0) [0.0|125.0] "m/s"
    )
    ...
    x_orient_ang: float = (
        -180.0
    )  # FLR2SignalStatusXOrientAng: 200|12@1+ (0.1,-180.0) [-180.0|180.0] "Deg"
    y_orient_ang: float = (
        -180.0
    )  # FLR2SignalStatusYOrientAng: 216|12@1+ (0.1,-180.0) [-180.0|180.0] "Deg"
    z_orient_ang: float = (
        -180.0
    )  # FLR2SignalStatusZOrientAng: 232|12@1+ (0.1,-180.0) [-180.0|180.0] "Deg"

    # Angle Corrections
    azi_ang_cor: float = (
        -12.8
    )  # FLR2SignalStatusAziAngCor: 248|8@1+ (0.1,-12.8) [-12.8|12.7] "Deg"
    ele_ang_cor: float = (
        -12.8
    )  # FLR2SignalStatusEleAngCor: 320|8@1+ (0.1,-12.8) [-12.8|12.7] "Deg"

    # Position Offset (X-axis)
    x_axis_offs: float = (
        -25.0
    )  # FLR2SignalStatusXAxisOffs: 288|16@1+ (0.00145204372,-25.0) [-25.0|25.0] "m"

    # Calibration Status
    cal_prgrss_sts: int = (
        0  # FLR2SignalStatusCalPrgrsSts: 304|8@1+ (1.0,0.0) [0.0|255.0] "Unitless"
    )
    whl_comp_fact: float = 0.92  # FLR2SignalStatusWhlCompFact: 264|5@1+ (0.005,0.92) [0.92|1.075] "Unitless"

    # Software/Interface Versions
    if_vers_major: int = (
        0  # FLR2SignalStatusIfVersMajor: 212|4@1+ (1.0,0.0) [0.0|15.0] "Unitless"
    )
    if_vers_minor: int = (
        0  # FLR2SignalStatusIfVersMinor: 228|4@1+ (1.0,0.0) [0.0|15.0] "Unitless"
    )
    sw_vers_major: int = (
        0  # FLR2SignalStatusSwVersMajor: 184|8@1+ (1.0,0.0) [0.0|255.0] "Unitless"
    )
    sw_vers_minor: int = (
        0  # FLR2SignalStatusSwVersMinor: 192|8@1+ (1.0,0.0) [0.0|255.0] "Unitless"
    )

    # Status Information
    scan_id_sts: int = (
        0  # FLR2SignalStatusScanIDSts: 176|8@1+ (1.0,0.0) [0.0|255.0] "Unitless"
    )
    timestamp_status: bool = (
        False  # FLR2SignalStatusTimeStampStatus: 167|1@1+ (1,0) [0|1] ""
    )
...
OBJECT_CONFIG: Dict[Tuple[int, int], Tuple[int, int, str, str, str]] = {
    (0, 1): (
        ...
        0x8CF,
        "FlrFlr1canFr82",
        "FLR2RdrObject0To1ScanID",
        "FLR2RdrObject0To1MsgCntr",
    ),
    (2, 3): (
        0x142,
        0x8D0,
        "FlrFlr1canFr83",
        "FLR2RdrObject2To3ScanID",
        "FLR2RdrObject2To3MsgCntr",
    ),
    (4, 5): (
        0x144,
        0x8D1,
        "FlrFlr1canFr84",
        "FLR2RdrObject4To5ScanID",
        "FLR2RdrObject4To5MsgCntr",
    ),
    (6, 7): (
        0x146,
        0x8D2,
        "FlrFlr1canFr85",
        "FLR2RdrObject6To7ScanID",
        "FLR2RdrObject6To7MsgCntr",
    ),
    (8, 9): (
        0x148,
        0x8D3,
        "FlrFlr1canFr86",
        "FLR2RdrObject8To9ScanID",
        "FLR2RdrObject8To9MsgCntr",
    ),
    (10, 11): (
        0x14A,
        0x8D3,
        "FlrFlr1canFr86",
        "FLR2RdrObject10To11ScanID",
        "FLR2RdrObject10To11MsgCntr",
    ),
    (12, 13): (
        0x14C,
        0x8D4,
        "FlrFlr1canFr87",
        "FLR2RdrObject12To13ScanID",
        "FLR2RdrObject12To13MsgCntr",
    ),
    (14, 15): (
        0x14E,
        0x8D5,
        "FlrFlr1canFr88",
        "FLR2RdrObject14To15ScanID",
        "FLR2RdrObject14To15MsgCntr",
    ),
    (16, 17): (
        0x150,
        0x8D6,
        "FlrFlr1canFr89",
        "FLR2RdrObject16To17ScanID",
        "FLR2RdrObject16To17MsgCntr",
    ),
    (18, 19): (
        0x152,
        0x8D7,
        "FlrFlr1canFr90",
        "FLR2RdrObject18To19ScanID",
        "FLR2RdrObject18To19MsgCntr",
    ),
    (20, 21): (
        0x154,
        0x8A9,
        "FlrFlr1canFr91",
        "FLR2RdrObject20To21ScanID",
        "FLR2RdrObject20To21MsgCntr",
    ),
    (22, 23): (
        0x156,
        0x8AA,
        "FlrFlr1canFr92",
        "FLR2RdrObject22To23ScanID",
        "FLR2RdrObject22To23MsgCntr",
    ),
    (24, 25): (
        0x158,
        0x8AB,
        "FlrFlr1canFr93",
        "FLR2RdrObject24To25ScanID",
        "FLR2RdrObject24To25MsgCntr",
    ),
    (26, 27): (
        0x15A,
        0x8AC,
        "FlrFlr1canFr94",
        "FLR2RdrObject26To27ScanID",
        "FLR2RdrObject26To27MsgCntr",
    ),
    (28, 29): (
        0x15C,
        0x8AD,
        "FlrFlr1canFr95",
        "FLR2RdrObject28To29ScanID",
        "FLR2RdrObject28To29MsgCntr",
    ),
}


def create_object_property(
    obj_id: int, start_obj: int, end_obj: int, prop_index: int
) -> ObjectProperty:
...
def create_object_lists() -> Tuple[ObjectList, ...]:
    ...
    for (start_obj, end_obj), (
        arb_id,
        e2e_id,
        frame_name,
        scan_id,
        msg_cntr,
    ) in OBJECT_CONFIG.items():
        ...
        # Create object list
        obj_list = ObjectList(
            arb_id, e2e_id, frame_name, scan_id, msg_cntr, objects_prop
        )
...
_object_lookup: Dict[int, ObjectList] = {
    entry.arbitration_id: entry for entry in object_attribute_list
}


def update_object_data(
    decoded_message: dict, obj_prop: ObjectProperty, index: int
) -> None:
...
def process_RadarStatus_CAN0(
    radar_dbc: database.Database, message_radar
) -> FlrFlr1canFr96:
    """
    ...
    try:
        print(
            f"Received signal status frame 0x{SIGNAL_STATUS_CAN_ID:03X} with {len(message_radar.data)} bytes"
        )
        """       
        ...
        decoded_message = radar_dbc.decode_message(
            message_radar.arbitration_id, message_radar.data
        )
        ...
        def safe_get(signal_name, default_value, value_type=None):
            try:
                ...
                if value_type == int:
                    ...
                    return (
                        bool(int(float(actual_value)))
                        if isinstance(actual_value, (int, float, str))
                        else bool(actual_value)
                    )
                else:
                    return actual_value
                ...
                print(
                    f"Warning: Could not decode {signal_name}, using default {default_value}. Error: {e}"
                )
                return default_value
        ...
        radar_signal_status.internal_temp = safe_get(
            "FLR2SignalStatusInternalTemp", -40.0, float
        )  # type: ignore
        radar_signal_status.timestamp = safe_get("FLR2SignalStatusTimeStamp", 0, int)  # type: ignore
        radar_signal_status.y_axis_offs = safe_get(
            "FLR2SignalStatusYAxisOffs", -25.0, float
        )  # type: ignore
        radar_signal_status.z_axis_offs = safe_get(
            "FLR2SignalStatusZAxisOffs", -25.0, float
        )  # type: ignore
        radar_signal_status.x_axis_offs = safe_get(
            "FLR2SignalStatusXAxisOffs", -25.0, float
        )  # type: ignore
        radar_signal_status.ego_spd_est = safe_get(
            "FLR2SignalStatusEgoSpdEst", 0.0, float
        )  # type: ignore
        radar_signal_status.ego_yaw_rate_est = safe_get(
            "FLR2SignalStatusEgoYawRateEst", 0.0, float
        )  # type: ignore
        radar_signal_status.x_orient_ang = safe_get(
            "FLR2SignalStatusXOrientAng", -180.0, float
        )  # type: ignore
        radar_signal_status.y_orient_ang = safe_get(
            "FLR2SignalStatusYOrientAng", -180.0, float
        )  # type: ignore
        radar_signal_status.z_orient_ang = safe_get(
            "FLR2SignalStatusZOrientAng", -180.0, float
        )  # type: ignore
        radar_signal_status.azi_ang_cor = safe_get(
            "FLR2SignalStatusAziAngCor", -12.8, float
        )  # type: ignore
        radar_signal_status.ele_ang_cor = safe_get(
            "FLR2SignalStatusEleAngCor", -12.8, float
        )  # type: ignore
        radar_signal_status.cal_prgrss_sts = safe_get(
            "FLR2SignalStatusCalPrgrsSts", 0, int
        )  # type: ignore
        radar_signal_status.whl_comp_fact = safe_get(
            "FLR2SignalStatusWhlCompFact", 0.92, float
        )  # type: ignore
        radar_signal_status.if_vers_major = safe_get(
            "FLR2SignalStatusIfVersMajor", 0, int
        )  # type: ignore
        radar_signal_status.if_vers_minor = safe_get(
            "FLR2SignalStatusIfVersMinor", 0, int
        )  # type: ignore
        radar_signal_status.sw_vers_major = safe_get(
            "FLR2SignalStatusSwVersMajor", 0, int
        )  # type: ignore
        radar_signal_status.sw_vers_minor = safe_get(
            "FLR2SignalStatusSwVersMinor", 0, int
        )  # type: ignore
        radar_signal_status.scan_id_sts = safe_get("FLR2SignalStatusScanIDSts", 0, int)  # type: ignore
        radar_signal_status.timestamp_status = safe_get(
            "FLR2SignalStatusTimeStampStatus", False, bool
        )  # type: ignore
        radar_signal_status.flt_reason = safe_get("FLR2SignalStatusFltReason", 0, int)  # type: ignore
        radar_signal_status.comm_flt_reason = safe_get(
            "FLR2SignalStatusCommFltReason", 0, int
        )  # type: ignore
        ...
        radar_signal_status.interference = safe_get(
            "FLR2SignalStatusInterference", 0, int
        )  # type: ignore
        radar_signal_status.sys_fail_flag = safe_get(
            "FLR2SignalStatusSysFailFlag", False, bool
        )  # type: ignore
        radar_signal_status.rdr_sts = safe_get("FLR2SignalStatusRdrSts", False, bool)  # type: ignore
        radar_signal_status.rdr_trans_act = safe_get(
            "FLR2SignalStatusRdrTransAct", False, bool
        )  # type: ignore
        radar_signal_status.signal_status_ub = safe_get(
            "FLR2SignalStatus_UB", False, bool
        )  # type: ignore

        print(
            f"Updated global radar_signal_status: Temp={radar_signal_status.internal_temp:.1f}°C, Counter={radar_signal_status.counter}"
        )
...
def process_ObjectList_CAN0(radar_dbc: database.Database) -> None:
    ...
    entry = _object_lookup.get(message_radar.arbitration_id)
    if entry is None:
        return
    ...
    update_object_data(
        decoded_message, entry.msg_obj_prop.second_obj_prop, index_entry + 1
    )
...
def process_CAN0_rx(radar_dbc: database.Database, can_bus_radar) -> RadarView:
    ...
    try:
        ...
        if message_radar.arbitration_id == SIGNAL_STATUS_CAN_ID:
            print(
                f"  → Signal Status Frame (0x45): {len(message_radar.data)} bytes received"
            )
        ...
        if message_radar.arbitration_id == SIGNAL_STATUS_CAN_ID:
            ...
            print(
                f"Radar CAN Message: ID=0x{message_radar.arbitration_id:03X} ({message_radar.arbitration_id}) | "
...
def process_CAN1_rx(can_bus_car) -> EgoMotion:
    ...
    try:
        ...
        # Process wheel speeds
            ...
            print(
                f"Vehicle CAN ID: 0x{message_car.arbitration_id:03X}, Data: {message_car.data.hex()}"
            )
