...
def draw_can_sniffer(screen: draw_2D_Surface):
    """Draw CAN sniffer data on screen"""
    if not can_sniffer.enabled or not can_sniffer.messages:
        return
    
    # Create semi-transparent overlay
    overlay = draw_2D_Surface(screen.get_size(), SRCALPHA)
    overlay.fill((0, 0, 0, 180))  # Semi-transparent black background
    screen.blit(overlay, (0, 0))
    
    # Draw title
    font_title = pygame_font.Font(pygame_font.get_default_font(), 24)
    title_text = font_title.render("CAN Bus Sniffer", True, white)
    ...
    for i, message in enumerate(can_sniffer.messages):
        if y_offset + line_height > screen.get_height() - 40:  # Leave space at bottom
            break
            
        # Alternate background colors for readability
        if i % 2 == 0:
            pygame_draw.rect(screen, (30, 30, 30), 
                           (10, y_offset - 2, screen.get_width() - 20, line_height))
    ...
    instructions = [
        "Press 'S' to toggle CAN Sniffer",
        f"Messages: {len(can_sniffer.messages)}/{can_sniffer.max_messages}",
        "ESC to exit"
    ]
    
    font_small = pygame_font.Font(pygame_font.get_default_font(), 12)
    for i, instruction in enumerate(instructions):
        text = font_small.render(instruction, True, yellow)
        screen.blit(text, (20, screen.get_height() - 60 + i * 15))
