...
class VisualizationThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            self.screen, self.ego_group, self.vehicle_group, self.ego_vehicle = init_draw()
            import pygame
            ...
            try:
                ...
                for event in events:
                    if event.type == QUIT:
                        self.running = False
                        break
                    elif event.type == KEYDOWN and hasattr(event, 'key') and event.key == 27:  # ESC
                        self.running = False
                        break
                ...
                if is_can_screen_enabled[0]:
                    ...
                    draw_exit_button(self.screen, self.screen.get_width() - 110, 10, 100, 40, gray, self._exit_callback, events)
                    ...
                    draw_extraInfo(self.screen, self.ego_motion_data, self.vehicle_group, radar_view.scan_id)
                    
                    # Draw swipe instructions
                    draw_swipe_instructions(self.screen, is_can_screen=False, is_radar_status_screen=False)
