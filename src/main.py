...
from rx import EgoMotion_data, process_rx_radar
...
from simulate import init_simulate_object_list, simulate_object_list
from defines import *

def periodic_task_60ms(interval_ms, task, stop_event:Event, *args, **kwargs):
    """
...
def main():
    try:
        ...
        init_simulate_object_list() # Initialize the object list for simulation
        ...
        stop_event_tx_car_thread = Event()     
        ...
        # can_bus_radar            
        tx_car_thread = Thread(target=periodic_task_60ms, args=(60, process_tx_radar, stop_event_tx_car_thread, main_can_bus_radar))
        # Start the thread
        tx_car_thread.start()
        ...
        while running:
            ...
            scanID = simulate_object_list(main_radar_dbc, main_can_bus_radar, main_can_bus_car)  
            # Process the RX data
            #scanID = process_rx_radar(main_radar_dbc, main_can_bus_radar, main_can_bus_car)
            ...
            draw_extraInfo(main_screen, EgoMotion_data, main_vehicle_group, scanID)
            
            # Update the display
            draw_update()  
            print(main_vehicle_group)
        # Signal the thread to stop
        stop_event_tx_car_thread.set()
        tx_car_thread.join()  # Wait for the thread to finish 
        deinit_draw()  
    except KeyboardInterrupt:
        stop_event_tx_car_thread.set()  # Ensure the thread stops on a keyboard interrupt
        tx_car_thread.join()
        deinit_com()
        deinit_draw()
